...
dpkg-deb --build "$STAGE/deb" "$OUT/print-bridge-server_${PACKAGE_VERSION}_${ARCH}.deb"
...
if command -v rpmbuild >/dev/null 2>&1; then
  ...
  sed "s/__VERSION__/$PACKAGE_VERSION/" "$ROOT/apps/server/packaging/rpm/print-bridge.spec" > "$RPMROOT/SPECS/print-bridge.spec"
