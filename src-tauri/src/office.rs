                ...
                match tokio::time::timeout(TIMEOUT_CLEANUP_TIMEOUT, cleanup_command.status()).await
                {
                    Ok(Ok(status)) if status.success() => {
                        tauri_plugin_log::log::warn!(
                            "Office timeout cleanup completed for {converter}"
                        );
                    }
                    Ok(Ok(_)) => {
                        tauri_plugin_log::log::warn!(
                            "Office timeout cleanup skipped or failed for {converter}"
                        );
                    }
                    Ok(Err(_)) | Err(_) => {
                        tauri_plugin_log::log::warn!(
                            "Office timeout cleanup could not run for {converter}"
                        );
                    }
                }
            }
