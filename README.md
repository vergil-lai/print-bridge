...
- 支持 PDF、图片和 raw commands 打印任务，图片会先转换成 PDF
- raw commands 使用内联 `data_base64`，PrintBridge 不解析 ESC/POS、ZPL、EPL、PCL、PostScript 等设备语言
- 每个 print job 可选传 `printer_name`；不传时使用设置中的默认打印机
...
PrintBridge 本地服务
  |
  | 下载 PDF / 图片，必要时转换 PDF；或原样提交 raw bytes
  v
系统打印队列
  |
  v
...
- raw commands，任务格式传 `raw`，内容使用 `data_base64`

raw 任务不支持 `file_url`、`paper`、`copies`。如果业务需要多份 raw 输出，应由业务系统生成多份 raw 指令或发送多个 job。
...
raw 任务不支持 `file_url`、`paper`、`copies`。
...
批量任务只表示一次接收多个 job，执行时仍然使用同一个串行队列。批量任务可以混合 PDF、image 和 raw。
